...
if [[ "${platform}" == "s32g274ardb2" ]]; then
...
elif [[ "${platform}" == "aptiv_cvc_fl" ]]; then
    # CVC 131 DCD

    # 0x00 DCD header (4 bytes)
    printf "\xd2\x00\x64\x60" | dd of="${dcd}" conv=notrunc bs=1
    # write command
    printf "\xcc\x00\x0c\x14\x40\x19\xc0\x00\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=4
    # check command
    printf "\xcf\x00\x0c\x14\x40\x19\xc0\x0c\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=16
    ...
    printf "\xcf\x00\x0c\x14\x40\x1a\x00\x0c\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=40
    # write command
    printf "\xcc\x00\x0c\x14\x40\x55\xa0\x00\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=52
    # check command
    printf "\xcf\x00\x0c\x04\x40\x55\xa0\x0c\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=64
    # write command
    printf "\xcc\x00\x0c\x14\x40\x55\xe0\x00\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=76
    # check command
    printf "\xcf\x00\x0c\x04\x40\x55\xe0\x0c\x00\x00\x00\x01" | dd of="${dcd}" conv=notrunc bs=1 seek=88
fi
